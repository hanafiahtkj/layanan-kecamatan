...
if (isset($_POST['print'])) {
    ...
    if (empty($bulan)) {
        $data  = $koneksi->query("SELECT * FROM legalisasi_imb WHERE YEAR(tgl_permohonan) = '$tahun'");
        $jml   = mysqli_num_rows($data);
        $info  = "Tahun " . $tahun;
    } else {
        $data  = $koneksi->query("SELECT * FROM legalisasi_imb WHERE MONTH(tgl_permohonan) = '$bulan' AND YEAR(tgl_permohonan) = '$tahun'");
        $jml   = mysqli_num_rows($data);
...
<body>
    ...
    <h2 align="center">
        <u>Laporan Legalisasi Permohonan Izin Mendirikan Bangunan</u>
    </h2>
...
$dompdf->render();
$dompdf->stream("lap-imb.pdf", array("Attachment" => false));
?>