...
if (isset($_POST['print'])) {
    ...
    if (empty($bulan)) {
        $data  = $koneksi->query("SELECT * FROM sktu_perpanjangan WHERE YEAR(tgl) = '$tahun'");
        $jml   = mysqli_num_rows($data);
        $info  = "Tahun " . $tahun;
    } else {
        $data  = $koneksi->query("SELECT * FROM sktu_perpanjangan WHERE MONTH(tgl) = '$bulan' AND YEAR(tgl) = '$tahun'");
        $jml   = mysqli_num_rows($data);
...
<body>
    ...
    <h2 align="center">
        <u>Laporan Surat Keterangan Tempat Usaha (Perpanjangan)</u>
    </h2>
...
$dompdf->render();
$dompdf->stream("lap-sktu-perpanjangan.pdf", array("Attachment" => false));
?>