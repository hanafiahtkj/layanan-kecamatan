...
if ($isValid)
{
    ...
    if(!$user)
    {
        $userrr = $koneksi->query("UPDATE masyarakat SET id_sso = '$id_sso' where id_masyarakat =".$id_app);
    }

    // PROSES LOGIN
    $user = $koneksi->query("SELECT * FROM masyarakat where id_masyarakat =".$id_app)->fetch_array();
